...
var utils = {
    ...
    subtractArray: function (arrA, arrB) {
        return arrA.concat().filter(function (a) {
    ...
    eachRole: function (grants, callback) {
        utils.eachKey(grants, function (name) {
            return callback(grants[name], name);
        });
    },
    /**
     *
            ...
            if (!utils.validName(resourceName, false)) {
                if (resourceName === '$extend') {
                    ...
                    if (!utils.isFilledStringArray(extRoles)) {
                        throw new core_1.AccessControlError(
                            'Invalid extend value for role "' +
                                roleName +
                                '": ' +
                                JSON.stringify(extRoles),
                        );
                    } else {
    ...
    getInspectedGrants: function (o) {
        ...
        if (type === 'object') {
            ...
            o.forEach(function (item) {
                return utils.commitToGrants(grants, item, true);
            });
        } else {
            throw new core_1.AccessControlError(
    ...
    normalizeQueryInfo: function (query) {
        if (utils.type(query) !== 'object') {
            throw new core_1.AccessControlError(
                'Invalid IQueryInfo: ' + typeof query,
            );
        }
        // clone the object
    ...
    normalizeAccessInfo: function (access, all) {
        ...
        if (all) access = utils.normalizeActionPossession(access);
        return access;
    },
    /**
    ...
    getRoleHierarchyOf: function (grants, roleName, rootRole) {
        ...
        role.$extend.forEach(function (exRoleName) {
            ...
            if (rootRole && rootRole === exRoleName) {
                throw new core_1.AccessControlError(
                    'Cross inheritance is not allowed. Role "' +
                        exRoleName +
                        '" already extends "' +
                        rootRole +
                        '".',
            ...
            var ext = utils.getRoleHierarchyOf(
                grants,
                exRoleName,
                rootRole || roleName,
            );
    ...
    extendRole: function (grants, roles, extenderRoles) {
        ...
        if (arrExtRoles.length === 0) {
            throw new core_1.AccessControlError(
                'Cannot inherit invalid role(s): ' +
                    JSON.stringify(extenderRoles),
            );
        }
        var nonExistentExtRoles = utils.getNonExistentRoles(
            grants,
            arrExtRoles,
        );
        ...
        roles.forEach(function (roleName) {
            ...
            var crossInherited = utils.getCrossExtendingRole(
                grants,
                roleName,
                arrExtRoles,
            );
